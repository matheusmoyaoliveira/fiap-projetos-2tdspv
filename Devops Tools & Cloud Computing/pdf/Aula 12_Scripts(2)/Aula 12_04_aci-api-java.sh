...
az container create \
  ...
  --memory 1 \
  --os-type Linux \
  --dns-name-label api-java-container-$rm \
  --ports 8080 \
  ...
  --environment-variables \
    SPRING_DATASOURCE_URL=$(az keyvault secret show --name spring-datasource-url --vault-name $keyVaultName --query value -o tsv | sed "s/mysql-dimdim/$mysqlPublicIP/") \
...
#
#fqdnJava=$(az container show --resource-group rg-money-hub --name api-java --query ipAddress.fqdn --output tsv)
